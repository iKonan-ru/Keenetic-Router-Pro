...
class KeeneticMeshPortSpeedSensor(MeshEntity, SensorEntity):
    ...
    _attr_entity_category = EntityCategory.DIAGNOSTIC
    _attr_suggested_display_precision = 0
    # Mesh nodes often have 4+ ports and most users only care about
    # one. Diagnostic-category sensors stay out of the main UI by
    # default, but they still register in the entity registry — so
    # we keep these disabled by default and let interested users
    # turn them on per-port. Matches what makes sense for the
    # controller-side equivalent once you scale to multi-port mesh
    # gear; ports the user doesn't care about don't grow the entity
    # list unnecessarily.
